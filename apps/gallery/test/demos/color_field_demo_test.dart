import 'package:flutter/widgets.dart';
import 'package:flutter_test/flutter_test.dart';
import 'package:hero_ui/hero_ui.dart';
import 'package:hero_ui_gallery/src/demo.dart';
import 'package:hero_ui_gallery/src/demos/color_field_demo.dart';
import 'package:hero_ui_gallery/src/pages/component_page.dart';

void main() {
  testWidgets('color field demo renders the playground and every example', (
    WidgetTester tester,
  ) async {
    tester.view.physicalSize = const Size(390, 6000);
    tester.view.devicePixelRatio = 1;
    addTearDown(tester.view.reset);
    final ComponentDemo demo = colorFieldDemo;
    await tester.pumpWidget(
      HeroApp(
        debugShowCheckedModeBanner: false,
        home: SingleChildScrollView(
          child: Column(
            children: <Widget>[
              PlaygroundView(playground: demo.playground!),
              for (final DemoExample example in demo.examples)
                Builder(builder: example.builder),
            ],
          ),
        ),
      ),
    );
    await tester.pumpAndSettle();
    expect(tester.takeException(), isNull);
    expect(find.byType(HeroColorField), findsWidgets);
    expect(demo.examples.map((DemoExample e) => e.title), <String>[
      'Usage',
      'Variants',
      'On Surface',
      'With Description',
      'Required Field',
      'Disabled State',
      'Full Width',
      'Validation',
      'Channel Editing',
      'Controlled',
      'Form Example',
      'Render Function',
      'Customization',
    ]);
    expect(find.text('Current: #7F007F'), findsOneWidget);
    expect(find.text('Please enter a valid hex color'), findsOneWidget);

    // Clear empties the controlled field.
    await tester.tap(find.text('Clear'));
    await tester.pumpAndSettle();
    expect(find.text('Current value: (empty)'), findsOneWidget);
    for (final DemoExample example in demo.examples) {
      expect(example.code, isNotEmpty);
    }
  });
}
